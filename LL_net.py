...
## extact the overlapped peaks.
    ...
    for label0 in labelset:
        ...
        S = np.exp(S)
        S.index=N_overlap
        S.to_csv('cell_type_specific_TF_RE_binding_'+label0+'.txt',sep='\t')
...
def trans_regulatory( RNA_file,ATAC_file, labels):  
    import pandas as pd
    from scipy.sparse import coo_matrix
    O_overlap=merge_columns_in_bed_file('Region_overlap.bed',1)
    N_overlap=merge_columns_in_bed_file('Region_overlap.bed',4)
    N_Peak=merge_columns_in_bed_file('Region.bed',1)
    O_all=merge_columns_in_bed_file('Peaks_0.bed',1)
    Binding=pd.read_csv('cell_population_TF_RE_binding.txt',sep='\t',index_col=0)
    cis=pd.read_csv('cell_population_cis_regulatory.txt',sep='\t',header=None)
...
# Map the column names and row names to integer indices in the DataFrame
    ...
    TF_TG = pd.read_csv('Primary_TF_TG.txt',sep='\t')
    ...
    TF_TG["col_index"] = TF_TG["TG"].map(row_dict)
    TF_TG["row_index"] = TF_TG["TF"].map(col_dict)
    ...
    sparse_S.colnames = TFset
    sparse_S.rownames = TGset
    TF_TG=sparse_S.toarray()
    S=np.matmul(Binding.T, cis).T.multiply(TF_TG)
    S.index=TGset
    S.to_csv('cell_population_trans_regulatory.txt',sep='\t')

def cell_type_specific_trans_regulatory(RNA_file,ATAC_file, labels):  
    import pandas as pd
    from scipy.sparse import coo_matrix
    with open('label.txt', 'r') as file:
    # Read the lines of the file and remove any trailing whitespace
    ...
    labelset=list(set(label))
    for label0 in labelset:
        Binding=pd.read_csv('cell_type_specific_TF_RE_binding_',label0,'.txt',sep='\t',index_col=0)
        cis=pd.read_csv('cell_type_specific_cis_regulatory_',+label0+'.txt',sep='\t',header=None)
        cis.columns=['RE','TG','Score']
        TGset=cis['TG'].unique()
        REset=Binding.index
        TFset=Binding.columns
...
# Map the column names and row names to integer indices in the DataFrame
        ...
        S=np.matmul(Binding.T, cis).T.multiply(TF_TG)
        S.index=TGset
        S.to_csv('cell_type_specific_trans_regulatory_'+label0,'.txt',sep='\t')
