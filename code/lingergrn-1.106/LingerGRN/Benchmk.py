...
def bm_trans(TFName, Method_name, Groundtruth, Infer_trans, outdir, filetype):
    ...
    for i in range(len(Infer_trans)):
        ...
        # Plot the ROC curve
        plt.plot(
            fpr, tpr, color=colors[i], label=Method_name[i] + ": (AUC = %0.2f)" % auc
        )
    ...
    plt.savefig(
        outdir + "trans_roc_curve" + TFName + ".png", format="png", bbox_inches="tight"
    )
    plt.show()
    plt.close()
    ...
    for i in range(len(Infer_trans)):
        ...
        plt.plot(
            ...
            precision,
            color=colors[i],
            label=Method_name[i] + ": (AUPR ratio = %0.2f)" % auprr,
        )
        plt.ylim([0.0, 0.6])
        plt.xlabel("Recall")
    ...
    plt.savefig(
        outdir + "trans_pr_curve" + TFName + ".png", format="png", bbox_inches="tight"
    )
    plt.show()
    plt.close()
