...
def get_simulation(outdir, chrall, data_merge, GRNdir, Exp, Opn, Target, idx):
    ...
    for i in range(23):
        ...
        for ii in tqdm(range(N)):
            ...
            if index_all in netall.keys():
                res = LINGER_simulation(
                    ii, gene_chr, TFindex, Exp, REindex, Opn, netall, index_all
                )
                output[index_all, :] = (
                    res.detach()
                    .numpy()
    ...
    output1 = pd.DataFrame(
        output, index=data_merge.loc[range(Target.shape[0])]["Symbol"]
    )
    return output1
...
def diff_umap(TFko, TFName, save, outdir, embedding, perturb, original, Input_dir):
    ...
    plt.scatter(
        ...
        cmap=cmap,
        s=2,
        vmin=-np.abs(continuous_values).max(),
        vmax=np.abs(continuous_values).max(),
    )
    anno = label
    ...
    for cluster in unique_clusters:
        ...
        cluster_center = (
            np.mean(embedding[indices, 0]),
            np.mean(embedding[indices, 1]),
        )
        plt.text(
            cluster_center[0],
            cluster_center[1],
    ...
    if save == True:
        plt.savefig(
            outdir + TFko + "_KO_Diff_exp_Umap_" + TFName + ".png",
            format="png",
            bbox_inches="tight",
        )
...
def Umap_direct(TFko, Input_dir, embedding, D, save, outdir):
    ...
    ax.quiver(
        ...
        * D[
            idx, 0
        ],  # Assuming gene_index is the index of the gene you are interested in
        2
        * D[
            idx, 1
        ],  # Assuming gene_index+1 is the index of another gene for the y-component
        scale=30,
        scale_units="inches",
        alpha=0.5,
    )
    if save == True:
        plt.savefig(
            outdir + TFko + "_KO_Differentiation_Umap.png",
            format="png",
            bbox_inches="tight",
        )
    plt.show()
