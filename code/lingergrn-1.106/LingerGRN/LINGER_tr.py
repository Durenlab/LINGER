...
# cpu is more efficient in this case.
...
def sc_nn(
    ...
    if (len(REidxtemp) == 1) & (REidxtemp[0] == "nan"):
        ...
        adj_matrix = np.zeros(
            [len(TFidxtemp) + len(REidxtemp), len(TFidxtemp) + len(REidxtemp)]
        )
    ...
    targets = torch.tensor(Target[gene_idx, :]).to(device)
    ...
    if (len(REidxtemp) > 0) & (len(REidxtemp_b) > 0):
        ...
        if merge_RE["id_b"].isna().sum() == 0:
            good = 1
            indexall = (
                merge_TF["id_b"].values.tolist()
                + (merge_RE["id_b"].values + merge_TF.shape[0]).tolist()
            )
        else:
            good = 0
    else:
    ...
    if good == 1:
        ...
        y_pred_all = 0 * (y + 1 - 1)
        y_pred_all1 = 0 * (y + 1 - 1)
        y_pred_all1 = y_pred_all1.cpu().numpy().reshape(-1)
        X_tr = inputs.T
        y_tr = y
        torch.manual_seed(seed_value)
        net = Net(input_size, activef).to(device)
        ...
        for i in range(100):
            ...
            lap_reg = alpha * torch.trace(
                torch.mm(torch.mm(net.fc1.weight, L), net.fc1.weight.t())
            )
            loss = mse_loss(y_pred, y_tr) + l1_norm * l1_lambda + l2_bulk + lap_reg
            Loss0[i, 0] = loss.detach().cpu().numpy()
            # Perform backpropagation
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def get_TSS(GRNdir, genome, TSS_dis):
    ...
    Tssdf[["chr", "1M-", "1M+", "symbol", "TSS", "strand"]].to_csv(
        "data/TSS_extend_1M.txt", sep="\t", index=None
    )
...
def sc_nn_NN(ii, RE_TGlink_temp, Target, Exp, Opn, l1_lambda, activef):
    ...
    inputs = np.vstack((TFtemp, REtemp))
    targets = torch.tensor(Target.loc[RE_TGlink_temp[0], :]).to(device)
    inputs = torch.tensor(inputs, dtype=torch.float32).to(device)
...
def load_data_scNN(GRNdir, species):
    ...
    if species == "New":
        ...
        Match2 = pd.read_csv(
            GRNdir + "Match_TF_motif_" + species + ".txt", header=None, sep="\t"
        )
...
def RE_TG_dis(outdir):
    ...
    peakList2 = [
        temp.split(":")[1].split("-")[0] for temp in peakList[0].values.tolist()
    ]
    peakList3 = [
        temp.split(":")[1].split("-")[1] for temp in peakList[0].values.tolist()
    ]
    peakList["chr"] = peakList1
    peakList["start"] = peakList2
    peakList["end"] = peakList3
    peakList[["chr", "start", "end"]].to_csv(
        current_directory + "/data/Peaks.bed", sep="\t", header=None, index=None
    )
    TSS_1M = pd.read_csv(
        current_directory + "/data/TSS_extend_1M.txt", sep="\t", header=0
    )
    TSS_1M.to_csv(
        current_directory + "/data/TSS_extend_1M.bed", sep="\t", header=None, index=None
    )
...
def training(GRNdir, method, outdir, activef, species):
    if method == "LINGER":
        ...
        Exp, idx, Opn, adj_matrix_all, Target, data_merge, TF_match = load_data(
            GRNdir, outdir
        )
...
def get_TSS_ensembl(genome_short, gtf_file, GRNdir):
    ...
    Tssdf.to_csv(
        GRNdir + "TSS_" + genome_short + ".txt", sep="\t", index=None, header=0
    )
