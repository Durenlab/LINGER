...
def pseudo_bulk(adata_RNA, adata_ATAC, singlepseudobulk):
    ...
    connectivities = pd.DataFrame(
        connectivities.toarray(), index=adata_RNA.obs["barcode"].tolist()
    )
    ...
    TG_filter1 = pd.DataFrame(
        TG_filter1.T, columns=allindex, index=adata_RNA.raw.var["gene_ids"].tolist()
    )
    RE_filter1 = pd.DataFrame(
        RE_filter1.T, columns=allindex, index=adata_ATAC.raw.var["gene_ids"].tolist()
    )
