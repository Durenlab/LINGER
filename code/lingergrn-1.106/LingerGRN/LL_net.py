...
def cosine_similarity_0(X):
    norm_term = np.linalg.norm(
        X, axis=1
    )  # calculate the norm term once rather than recalculate
...
def list2mat(df, i_n, j_n, x_n):
    ...
    matrix = coo_matrix(
        (df[x_n], (row_indices, col_indices)), shape=(len(REs), len(TFs))
    )
...
def get_TF_RE(
    data_merge_temp, j, net_all, TFindex, TFName, REindex, REName, torch_cosine=False
):
...
def load_TFbinding(GRNdir, O_overlap, O_overlap_u, O_overlap_hg19_u, chrN):
    TFbinding = pd.read_csv(
        GRNdir + "TF_binding_" + chrN + ".txt", sep="\t", index_col=0
    )
    ...
    O_overlap_hg19_u_df = pd.DataFrame(
        range(len(O_overlap_hg19_u)), index=O_overlap_hg19_u
    )
...
def TF_RE_LINGER_chr(chr, outdir):
    ...
    for ii in tqdm(range(times)):
        ...
        for j in range(ii * batchsize, (ii + 1) * batchsize):
            if (AAA[j] > 0) & (AAA[j] < 10):
                result = get_TF_RE(
                    data_merge_temp, j, net_all, TFindex, TFName, REindex, REName
                )
        ...
        for j in range(ii * batchsize, (ii + 1) * batchsize):
            ...
            if len(REidxtemp) > 0:
                if not torch_cosine:
                    ...
                    corr_matrix = (
                        cosine_similarity_0_torch(temps.t()).detach().cpu().numpy()
                    )
                corr_matrix = corr_matrix[: len(TFidxtemp), len(TFidxtemp) :]
                # result={'TF':[],'RE':[],'score':[]}
                # result = pd.DataFrame(result)
...
def load_data_scNN(GRNdir, genome):
    ...
    if genome in genome_map.index:
        Match2 = pd.read_csv(
            GRNdir
            + "Match_TF_motif_"
            + genome_map.loc[genome]["species_ensembl"]
            + ".txt",
            sep="\t",
            header=0,
        )
    ...
    RE_TGlink = (
        RE_TGlink.groupby("gene").apply(lambda x: x["RE"].values.tolist()).reset_index()
    )
...
def TF_RE_binding(
    GRNdir, adata_RNA, adata_ATAC, genome, method, outdir, torch_cosine=True
):
    ...
    if method == "scNN":
        ...
        for jj in tqdm(range(0, len(chrlist))):
            ...
            result_scnn = TF_RE_scNN(
                TFName, geneName, net_all, RE_TGlink1, REName, torch_cosine=torch_cosine
            )
            result_scnn.to_csv(
                outdir + chrtemp + "_cell_population_TF_RE_binding.txt", sep="\t"
            )
...
def load_TFbinding_scNN(GRNdir, outdir, genome):
    ...
    TF_binding1 = TF_binding1.merge(
        Match2, how="inner", left_on="motif", right_on="Motif"
    )
...
def cell_type_specific_TF_RE_binding_chr(
    ...
    RE = pd.DataFrame(
        temp.T, index=adata_ATAC.var["gene_ids"].values, columns=["values"]
    )
    temp = adata_RNA.X[np.array(label) == celltype, :].mean(axis=0)
    TG = pd.DataFrame(
        temp.T, index=adata_RNA.var["gene_ids"].values, columns=["values"]
    )
    del temp
    ## cell annotation
    ## extact the overlapped peaks.
    ...
    if method == "LINGER":
        ...
        if len(other_RE) > 0:
            B_arr = pd.DataFrame(
                np.zeros((len(other_RE), mat.shape[1])),
                columns=mat.columns,
                index=other_RE,
            )
    ...
    # RE_cluster = RE_cluster[:,None]
    S = (np.log(RE_cluster + 0.1) + np.log(mat + TFbinding + 0.1)).T + np.log(
        TF_cluster + 0.1
    )
    S = np.exp(S.T)
    S.index = N_overlap
...
def cell_type_specific_TF_RE_binding(
    GRNdir, adata_RNA, adata_ATAC, genome, celltype, outdir, method
):
    label = adata_RNA.obs["label"].values.tolist()
    labelset = list(set(label))
    if (celltype == "all") & (method != "scNN"):
        ...
        for label0 in labelset:
            print(
                "Generating cell type specitic TF binding potential for cell type "
                + str(label0)
                + "..."
            )
            result = pd.DataFrame()
            for i in tqdm(range(22)):
                chrN = "chr" + str(i + 1)
                mat = pd.read_csv(
                    outdir + chrN + "_cell_population_TF_RE_binding.txt",
                    sep="\t",
                    index_col=0,
                    header=0,
                )
            ...
            result.to_csv(
                outdir + "cell_type_specific_TF_RE_binding_" + str(label0) + ".txt",
                sep="\t",
            )
    elif method != "scNN":
        result = pd.DataFrame()
        ...
        result.to_csv(
            outdir + "cell_type_specific_TF_RE_binding_" + str(celltype) + ".txt",
            sep="\t",
        )
    elif (celltype == "all") & (method == "scNN"):
        A = pd.read_csv(
            outdir + "cell_population_TF_RE_binding.txt",
            sep="\t",
            header=0,
            index_col=0,
        )
        ...
        for label0 in labelset:
            ...
            RE = pd.DataFrame(
                temp, index=adata_ATAC.var["gene_ids"].values, columns=["values"]
            )
            temp = adata_RNA.X[np.array(label) == label0, :].mean(axis=0).T
            TG = pd.DataFrame(
                temp, index=adata_RNA.var["gene_ids"].values, columns=["values"]
            )
            RE = RE.loc[REs]
...
def cis_shap(chr, outdir):
    ...
    TG_2 = []
    score_2 = []
    data_merge_temp, geneName, REindex, TFindex, shap_all, TFName, REName = load_shap(
        chr, outdir
    )
...
def load_RE_TG(GRNdir, chrN, O_overlap_u, O_overlap_hg19_u, O_overlap):
    ...
    primary_s["RE"] = primary_s["RE"].apply(
        lambda x: x.split("_")[0] + ":" + x.split("_")[1] + "-" + x.split("_")[2]
    )
...
def load_RE_TG_distance(
    GRNdir, chrN, O_overlap_hg19_u, O_overlap_u, O_overlap, TGoverlap
):
    ...
    # Create the sparse matrix using coo_matrix
    sparse_dis = coo_matrix(
        (values, (row_indices, col_indices)), shape=(len(O_overlap_u), len(TGoverlap))
    )
...
def load_RE_TG_scNN(outdir):
    ...
    TGs = dis["gene"].unique()
    cis = pd.read_csv(
        outdir + "cell_population_cis_regulatory.txt", sep="\t", header=None
    )
...
def cis_reg(GRNdir, adata_RNA, adata_ATAC, genome, method, outdir):
    ...
    result.to_csv(
        outdir + "cell_population_cis_regulatory.txt", sep="\t", header=None, index=None
    )


def cell_type_specific_cis_reg_chr(
    GRNdir, adata_RNA, adata_ATAC, genome, chrN, celltype, outdir
):
    ...
    Score = (
        csc_matrix(RE_temp)
        .multiply(sparse_S.values)
        .multiply(sparse_dis.values)
        .multiply(csc_matrix(TG_temp.T))
        .toarray()
    )
    Score = pd.DataFrame(Score, index=N_overlap, columns=TGoverlap)
    Score = Score.groupby(Score.index).max()
...
def cell_type_specific_cis_reg_scNN(distance, cisGRN, RE, TG, REs, TGs):
    ...
    RE_temp = RE_temp / RE_temp.mean() + 0.1
    Score = (
        (cisGRN.multiply(csr_matrix(RE_temp)))
        .multiply(distance)
        .multiply(csr_matrix(TG_temp.T))
    )
...
def cell_type_specific_cis_reg(
    ...
    if (celltype == "all") & (method != "scNN"):
        for label0 in labelset:
            ...
            for i in tqdm(range(23)):
                chrN = chrom[i]
                temp = cell_type_specific_cis_reg_chr(
                    GRNdir, adata_RNA, adata_ATAC, genome, chrN, label0, outdir
                )
                results.append(temp)
            chrN = "chrX"
            temp = cell_type_specific_cis_reg_chr(
            ...
            result = pd.concat(results, axis=0, join="outer")
            result.to_csv(
                outdir + "cell_type_specific_cis_regulatory_" + str(label0) + ".txt",
                sep="\t",
                header=None,
                index=None,
            )
        ...
        result = pd.concat(results, axis=0, join="outer")
        result.to_csv(
            outdir + "cell_type_specific_cis_regulatory_" + celltype + ".txt",
            sep="\t",
            header=None,
            index=None,
        )
        ...
        for label0 in labelset:
            ...
            result.to_csv(
                outdir + "cell_type_specific_cis_regulatory_" + label0 + ".txt",
                sep="\t",
                header=None,
                index=None,
            )
...
def load_TF_TG(GRNdir, TFset, TGset):
    ...
    for i in a:
        ...
        sparse_S = coo_matrix(
            (values, (row_indices, col_indices)), shape=(len(TGset), len(TFset))
        )
...
def trans_reg(GRNdir, method, outdir, genome):
    ...
    if method == "baseline":
        Binding = pd.read_csv(
            outdir + "cell_population_TF_RE_binding.txt", sep="\t", index_col=0
        )
...
def cell_type_specific_trans_reg(GRNdir, adata_RNA, celltype, outdir):
    ...
    if celltype == "all":
        for label0 in labelset:
            ...
            S = pd.DataFrame(S, index=TGset, columns=TFset)
            S.to_csv(
                outdir + "cell_type_specific_trans_regulatory_" + str(label0) + ".txt",
                sep="\t",
            )
    else:
        Binding = pd.read_csv(
            outdir + "cell_type_specific_TF_RE_binding_" + celltype + ".txt",
            sep="\t",
            index_col=0,
        )
        ...
        S = pd.DataFrame(S, index=TGset, columns=TFset)
        S.to_csv(
            outdir + "cell_type_specific_trans_regulatory_" + celltype + ".txt",
            sep="\t",
        )
