...
def gene_expression(GRNdir, TG_pseudobulk, outdir):
    ...
    pd.DataFrame(List).to_csv(
        outdir + "Symbol.txt", sep="\t", header=False, index=False
    )
    pd.DataFrame(A.columns).to_csv(
        outdir + "Col.txt", sep="\t", index=False, header=False
    )
    return List, A
...
def TF_expression(TFName, List, Match2, A, outdir):
    ...
    pd.DataFrame(TFName).to_csv(
        outdir + "TFName.txt", sep="\t", header=False, index=False
    )
    return TFName
...
def load_corr_RE_TG(List, Element_name, Element_name_bulk, outdir):
    Element_gene = pd.read_csv(
        outdir + "hg19_Peak_hg19_gene_u.txt", delimiter="\t", header=None
    )
    choosL = List
    index_ElementName = pd.DataFrame(
        np.arange(0, len(Element_name)), index=Element_name
    )
    index_Element_name_bulk = pd.DataFrame(
        np.arange(0, len(Element_name_bulk)), index=Element_name_bulk
    )
    index_Element_name_bulk = index_Element_name_bulk.groupby(
        index_Element_name_bulk.index
    ).min()
    ...
    Element_gene["id_s"] = index_ElementName.loc[Element_gene["Element_name_s"]][
        0
    ].values
    Element_gene["id_b"] = index_Element_name_bulk.loc[Element_gene["Element_name_b"]][
        0
    ].values
...
def load_motifbinding_chr(chrN, GRNdir, motifWeight, outdir):
    Motif_binding_temp = pd.read_csv(
        GRNdir + "MotifTarget_Matrix_" + chrN + ".txt", sep="\t", index_col=0
    )
    REs = Motif_binding_temp.index
    march_hg19_Regrion = pd.read_csv(
        outdir + "MotifTarget_hg19_hg38_" + chrN + ".txt", sep="\t", header=None
    )
    ...
    Motif_binding_temp = np.zeros(
        [march_hg19_Regrion.shape[0], Motif_binding_temp.shape[1]]
    )
    Motif_binding_temp = Motif_binding_temp1.loc[march_hg19_Regrion[1].values].values
    Motif_binding_temp = pd.DataFrame(
        Motif_binding_temp,
        index=march_hg19_Regrion[0].values,
...
def load_TFbinding(GRNdir, motifWeight, Match2, TFName, Element_name, outdir):
    ...
    for chrN in tqdm(chrall):
        ...
        motif_binding = pd.concat(
            [motif_binding, Motif_binding_temp1], join="outer", axis=0
        )
    ...
    Motif_binding = pd.DataFrame(
        Motif_binding, index=motif_binding.columns, columns=Element_name
    )
    Match2 = Match2[np.isin(Match2[:, 1], TFName), :]
    ...
    TF_binding_n = pd.DataFrame(
        TF_binding_n.T, index=Element_name, columns=TF_binding.index
    )
...
def extract_overlap_regions(genome, GRNdir, outdir, method):
    ...
    if method == "LINGER":
        if genome == "hg38":
            ...
            a_with_b[[6, 7, 8, 0, 1, 2]].to_csv(
                outdir + "match_hg19_peak.bed", sep="\t", header=None, index=None
            )
        if genome == "hg19":
            a = pybedtools.example_bedtool(outdir + "Region.bed")
        ...
        a_with_b = pd.read_csv(outdir + "temp.bed", sep="\t", header=None)
        a_with_b = a_with_b[
            (a_with_b[1].values == a_with_b[7].values)
            & (a_with_b[2].values == a_with_b[8].values)
        ]
        a_with_b_n = pd.DataFrame(
            {
                "column1": a_with_b[0]
                + ":"
                + a_with_b[1].astype(str)
                + "-"
                + a_with_b[2].astype(str),
                "column2": a_with_b[3]
                + ":"
                + a_with_b[4].astype(str)
                + "-"
                + a_with_b[5].astype(str),
                "column3": a_with_b[9],
            }
        )
        a_with_b_n = a_with_b_n.drop_duplicates()
        a_with_b_n.to_csv(
            outdir + "hg19_Peak_hg19_gene_u.txt", sep="\t", header=None, index=None
        )
        chr_all = ["chr" + str(i + 1) for i in range(22)]
        chr_all.append("chrX")
        for chrtemp in chr_all:
            a = pybedtools.example_bedtool(outdir + "match_hg19_peak.bed")
            b = pybedtools.example_bedtool(
                GRNdir + "MotifTarget_matrix_" + chrtemp + ".bed"
            )
            a_with_b = a.intersect(b, wa=True, wb=True)
            a_with_b.saveas(outdir + "temp.bed")
            a_with_b = pd.read_csv(outdir + "temp.bed", sep="\t", header=None)
            ...
            a_with_b_n = pd.DataFrame(
                {
                    ...
                    "column2": a_with_b[6]
                    + ":"
                    + a_with_b[7].astype(str)
                    + "-"
                    + a_with_b[8].astype(str),
                }
            )
            a_with_b_n = a_with_b_n.drop_duplicates()
            a_with_b_n.to_csv(
                outdir + "MotifTarget_hg19_hg38_" + chrtemp + ".txt",
                sep="\t",
                header=None,
                index=None,
            )
            a = pybedtools.example_bedtool(
                GRNdir + genome + "_Peaks_" + chrtemp + ".bed"
            )
...
def preprocess(TG_pseudobulk, RE_pseudobulk, GRNdir, genome, method, outdir):
    ...
    if method == "LINGER":
        ...
        Element_name_bulk = pd.read_csv(
            GRNdir + "all_hg19.txt", delimiter="\t", header=None
        )
        ...
        merged_s, merged_b = load_corr_RE_TG(
            List, Element_name, Element_name_bulk, outdir
        )
        from tqdm import tqdm
        ...
        pd.DataFrame(out).to_csv(
            outdir + "index.txt", sep="\t", header=None, index=None
        )
        ...
        print(
            "Method:" + method + "is not found! Please set method as baseline or LINGER"
        )
...
def get_adata(matrix, features, barcodes, label):
    ...
    if len(barcodes[0].values[0].split("-")) == 2:
        adata.obs["sample"] = [
            int(string.split("-")[1]) for string in barcodes[0].values
        ]
    else:
        adata.obs["sample"] = 1
...
def get_adata_h5(adata_RNA, adata_ATAC, label):
    ...
    if len(adata_RNA.obs["barcode"].values[0].split("-")) == 2:
        adata_RNA.obs["sample"] = [
            int(string.split("-")[1]) for string in adata_RNA.obs["barcode"].values
        ]
        adata_ATAC.obs["sample"] = [
            int(string.split("-")[1]) for string in adata_ATAC.obs["barcode"].values
        ]
    else:
