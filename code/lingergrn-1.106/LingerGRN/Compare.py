...
def qq_pval(p1, names, celltype):
    ...
    plt.plot(
        np.arange(0, m + 0.01, 0.01), np.arange(0, m + 0.01, 0.01), "-r", linewidth=1
    )
...
def diff_Module(Exp_TG, metadata, S_TG, K):
    ...
    pvalue_all = pd.DataFrame(
        pvalue_all, index=["M" + str(i + 1) for i in range(K)], columns=celltype
    )
    tvalue_all = pd.DataFrame(
        tvalue_all, index=["M" + str(i + 1) for i in range(K)], columns=celltype
    )
    ...
    for i in range(GWASgene.shape[1]):
        ...
        for k in range(K):
            ...
            contingency_table = np.array(
                [
                    [Noverlap, counts[k + 1] - Noverlap],
                    [N - Noverlap, len(TGset) - N - counts[k + 1] + Noverlap],
                ]
            )
            odds_ratio, p_value = fisher_exact(contingency_table, alternative="greater")
            p_fisher[k, i] = odds_ratio
            odds_ratio_fisher[k, i] = p_value
    odds_ratio_fisher = pd.DataFrame(
        odds_ratio_fisher,
        index=["M" + str(j + 1) for j in range(K)],
        columns=["GWAS_" + (str(j + 1)) for j in range(GWASgene.shape[1])],
    )
    p_fisher = pd.DataFrame(
        p_fisher,
        index=["M" + str(j + 1) for j in range(K)],
        columns=["GWAS_" + (str(j + 1)) for j in range(GWASgene.shape[1])],
    )
...
def Module_trans(outdir, metadata, TG_pseudobulk, K, GWASfile=None):
    ...
    R1 = stats.zscore(trans_reg, 1)
    R1[np.isnan(R1)] = 0.0
    R2 = stats.zscore(trans_reg, 0)
    R2[np.isnan(R2)] = 0.0
    # R1[R1<0]=0;R2[R2<0]=0
    ...
    Exp = pd.DataFrame(
        TG_pseudobulk, index=TG_pseudobulk.index, columns=TG_pseudobulk.columns
    )
    Z = R1 + R2
    Z[Z < 0] = 0
...
def remove_covariate(TG_pseudobulk, aud_idx, celltype):
    ...
    for i in range(len(celltypeset)):
        ...
        Exp_norm1 = (
            Exp
            - np.dot(beta, Cov.T)
            + np.dot(np.reshape(beta[:, 0], (-1, 1)), np.ones((1, Exp.shape[1])))
        )
        Exp_norm1[Exp_norm1 < 0] = 0
        Exp_norm[:, celltype == celltypeset[i]] = Exp_norm1
    Exp_norm = pd.DataFrame(
        Exp_norm, columns=TG_pseudobulk.columns, index=TG_pseudobulk.index
    )
    return Exp_norm
...
def runWGCNA(celltypetemp, TG_pseudobulk_all, metadata):
    ...
    pyWGCNA_5xFAD.setMetadataColor("group", {0: "green", 1: "yellow"})
...
def driver_score(expression, aud_idx, GRN, outdir, adjust_method, corr_method):
    ...
    E = (
        np.reshape(rows, (rows.shape[0], 1))
        * np.reshape(cols, (1, cols.shape[0]))
        / rows.sum()
    )
    ...
    for i in range(len(allcelltype)):
        ...
        exp_temp = expression.iloc[
            :, aud_idx1[aud_idx1["celltype"] == allcelltype[i]].index
        ]
        aud_idx1 = aud_idx1[aud_idx1["celltype"] == allcelltype[i]]
        Mean1 = exp_temp[aud_idx1[aud_idx1["group"] == 1]["index"]].mean(
            axis=1
        ) + 10 ** (-6)
        Mean0 = exp_temp[aud_idx1[aud_idx1["group"] == 0]["index"]].mean(
            axis=1
        ) + 10 ** (-6)
